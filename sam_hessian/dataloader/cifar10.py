...
def get_cifar10(
    ...
    data_train, data_val = random_split(
        dataset=torchvision.datasets.CIFAR10(root=os.path.join('.', 'data'), train=True, download=True, transform=transform_train),
        lengths=split,
        generator=torch.Generator().manual_seed(42)
    )
    data_test = torchvision.datasets.CIFAR10(root=os.path.join('.', 'data'), train=False, download=True, transform=transform_test)
